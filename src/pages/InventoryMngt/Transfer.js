...
class Transfer extends Component {
  ...
  componentDidMount() {
    ...
    this.props.dispatch(dashboardActions.getTransferList(data));
  }
  ...
  deleteUser = (data) => {
    ...
    confirmAlert({
      ...
      buttons: [
        {
          label: 'Yes',
          onClick: () => this.props.dispatch(dashboardActions.deleteTransfer(datatemp))
        },
        {
          label: 'No'
        }
  ...
  render() {
    ...
    let { getTransferList, getAllCourse, getAllStudent, getAllBatch } = dashboard;

    console.log('getTransferListgetTransferListgetTransferListgetTransferList', getTransferList);
          ...
          <section>
            ...
            <div className='bg-white border border-slate-300'>
              <div className='p-4 border-b border-slate-300'>
                <h1 className='text-xl font-normal'>Add New Transfer</h1>
              </div>
                ...
                <div className='grid grid-cols-1 gap-6 p-8 lg:grid-cols-4 md:grid-cols-2'>
                  <div>
                    ...
                    <input className={`w-full border px-4 py-1.5 rounded-xs focus:outline-none text-base text-gray-700 placeholder-gray-500 ${!this.state.errorBatch["sessionYear"] ? "'border-opacity-20 " : "order-opacity-100 border-red-500 bg-red-100"}`}
                      ...
                      onChange={this.inputChange} placeholder='Enter Date' type="text" />
                  ...
                  <div>
                    <label class=' text-sm font-normal text-gray-600 pb-4'>Return Due Date</label>
                    <input className={`w-full border px-4 py-1.5 rounded-xs focus:outline-none text-base text-gray-700 placeholder-gray-500 ${!this.state.errorBatch["amount"] ? "'border-opacity-20 " : "order-opacity-100 border-red-500 bg-red-100"}`}
                      ...
                      onChange={this.inputChange} placeholder='Enter Return Due Date' type="text" />
                  ...
                  <div>
                    <label class=' text-sm font-normal text-gray-600 pb-4'>Room</label>
                    <input className={`w-full border px-4 py-1.5 rounded-xs focus:outline-none text-base text-gray-700 placeholder-gray-500 ${!this.state.errorBatch["amount"] ? "'border-opacity-20 " : "order-opacity-100 border-red-500 bg-red-100"}`}
                      name="amount"
                      value={this.state.fieldsBatch && this.state.fieldsBatch["amount"] ? this.state.fieldsBatch["amount"] : ""}
                      onChange={this.inputChange} placeholder='Enter Room' type="text" />
                    {this.state.errorBatch["amount"] ?
                      <div className="text-yellow-600 invalid-feedback">
                        {this.state.errorBatch["amount"]}
                      </div>
                  ...
                  <div>
                    ...
                    <input className={`w-full border px-4 py-1.5 rounded-xs focus:outline-none text-base text-gray-700 placeholder-gray-500 ${!this.state.errorBatch["amount"] ? "'border-opacity-20 " : "order-opacity-100 border-red-500 bg-red-100"}`}
                      name="amount"
                      value={this.state.fieldsBatch && this.state.fieldsBatch["amount"] ? this.state.fieldsBatch["amount"] : ""}
                      onChange={this.inputChange} placeholder='Enter Alternate Phone' type="text" />
                    {this.state.errorBatch["amount"] ?
                      <div className="text-yellow-600 invalid-feedback">
                        {this.state.errorBatch["amount"]}
                      </div>
                ...
                <div className="overflow-hidden ">
                  <table className="min-w-full text-left bg-white border border-collapse border-slate-300">
                    ...
                    <tbody className='divide-y' >
                      ...
                      {
                        getTransferList && getTransferList.length > 0 ?
                          getTransferList.map((element, index) => (
                            <>
